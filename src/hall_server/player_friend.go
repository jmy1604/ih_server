...
func (this *Player) _friend_get_points(friend *Player, now_time int32) (get_points int32) {
	...
	if utils.GetRemainSeconds2NextDayTime(last_give_time, global_config.FriendRefreshTime) == 0 {
		...
		if get_points+this.db.FriendCommon.GetGetPointsDay() > global_config.FriendPointsGetLimitDay {
			get_points = global_config.FriendPointsGetLimitDay - this.db.FriendCommon.GetGetPointsDay()
		}
	}
	return
}
...
func (this *Player) agree_friend_ask(player_ids []int32) int32 {
	...
	for i := 0; i < len(player_ids); i++ {
		...
		if p.db.Friends.NumAll() >= global_config.FriendMaxNum {
			if len(player_ids) == 1 {
				log.Error("Player[%v] cant agree add friend %v", this.Id, p.Id)
...
func (this *Player) get_friend_points(friend_ids []int32) int32 {
	...
	for i := 0; i < len(friend_ids); i++ {
		...
		if get_point > 0 {
			...
			this.db.FriendCommon.IncbyGetPointsDay(get_point)
...
func (this *Player) friend_boss_challenge(friend_id int32) int32 {
	...
	this.check_and_add_friend_stamina()
	...
	if n > 0 && stamina >= global_config.FriendStaminaLimit {
		this.db.FriendCommon.SetLastGetStaminaTime(int32(time.Now().Unix()))
	}
	...
	// 最后一击
	if is_win {
		this.send_stage_reward(stage.RewardList, 5, 0)
		RealSendMail(nil, this.Id, MAIL_TYPE_SYSTEM, 1109, "", "", friend_boss_tdata.RewardLastHit, 0)
