...
func (this *Player) UpdateArenaScore(is_win bool) (score, add_score int32) {
	...
	if add_score > 0 {
		...
		if new_division != nil && new_division.Id > division.Id {
			RealSendMail(nil, this.Id, MAIL_TYPE_SYSTEM, 1104, "", "", new_division.RewardList, new_division.Id)
	...
	this.activitys_update(ACTIVITY_EVENT_ARENA_SCORE, func() int32 {
		if is_win {
			return 2
		} else {
			return 1
...
func (this *ArenaSeasonMgr) Init() bool {
	...
	if !this.day_checker.Init(dbc.ArenaSeason.GetRow().Data.GetLastDayResetTime(), global_config.ArenaDayResetTime, 1) {
